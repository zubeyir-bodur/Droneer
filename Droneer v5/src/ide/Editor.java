...
public class Editor extends JPanel {
   ...
   public Editor() {
         ...
         public void actionPerformed(ActionEvent e) {
            ...
            if (hasChanged) {
               int r = JOptionPane.showConfirmDialog(null,
                     "You did not save the current file, do you want to save it and create a new file?",
      ...
      newButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put((KeyStroke) newAct.getValue(Action.ACCELERATOR_KEY),
            "newAction");
      ...
      save.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put((KeyStroke) saveAct.getValue(Action.ACCELERATOR_KEY),
            "saveAction");
      ...
      undo.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put((KeyStroke) undoAct.getValue(Action.ACCELERATOR_KEY),
            "undoAction");
      ...
      redo.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put((KeyStroke) redoAct.getValue(Action.ACCELERATOR_KEY),
            "redoAction");
      ...
      open.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put((KeyStroke) openAct.getValue(Action.ACCELERATOR_KEY),
            "openAction");
      ...
      Action compileAct = new AbstractAction("Compile") {
         ...
         public void actionPerformed(ActionEvent e) {
            if (filename.equals(""))
               ...
               Object[] params = { "Current file is not saved." + " You need to save it before compiling."
                     + " Click OK to save&compile the file.", checkbox };
               int desire = JOptionPane.showConfirmDialog(null, params, "Unsaved File", JOptionPane.OK_CANCEL_OPTION,
      ...
      // Uncomment the following and comment the uncommented ones to make the hotkey
      // (currently F5) Ctrl + B
      // compileAct.putValue(Action.ACCELERATOR_KEY,
      ...
      compile.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_F5, 0),
            "compileAction");
      ...
      interactionsPanel = new InteractionsPanel(
            "Welcome to Droneer. Current file directory is : " + System.getProperty("user.dir") + "\\" + dir);
   ...
   /*
    * Action listener for saveAs button writes the text info to the saved file
    */
   class SaveAsListener implements ActionListener {
      public void actionPerformed(ActionEvent e) {
         int response = fileChooser.showSaveDialog(null);
         if (response == JFileChooser.APPROVE_OPTION) {
            // Set the label, file name and directory properties, then save the data at
            // those properties
            filename = fileChooser.getSelectedFile().getName();
            dir = fileChooser.getCurrentDirectory() + "";
            saveDataAs(text.getText(), dir + "\\" + filename);
